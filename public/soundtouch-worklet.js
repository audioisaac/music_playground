// Vendored from soundtouchjs v0.3.0 (LGPL) + a live AudioWorkletProcessor wrapper.
// Generated file — do not edit by hand. See REVISION 8 in the plan.
/*
...
// --- Live pitch-shift processor (mono in/out) ---
class SoundTouchShifter extends AudioWorkletProcessor {
  static get parameterDescriptors() {
    return [{ name: "pitch", defaultValue: 0, automationRate: "k-rate" }];
  }
  constructor() {
    super();
    this.st = new SoundTouch(); // tempo 1, pitch 1 by default (passthrough)
    this.lastPitch = 0;
    this.interleaved = new Float32Array(256); // mono -> stereo, 128 frames
    this.recv = new Float32Array(16384);
    this.ring = new Float32Array(32768); // mono output backlog
    this.ringLen = 0;
  }
  process(inputs, outputs, params) {
    ...
    if (input) {
      ...
      if (this.interleaved.length < len * 2) this.interleaved = new Float32Array(len * 2);
      for (let i = 0; i < len; i++) {
        this.interleaved[2 * i] = input[i];
        this.interleaved[2 * i + 1] = input[i];
      }
      this.st.inputBuffer.putSamples(this.interleaved, 0, len);
      this.st.process();
      const avail = this.st.outputBuffer.frameCount;
      if (avail > 0) {
        if (this.recv.length < avail * 2) this.recv = new Float32Array(avail * 2);
        const got = this.st.outputBuffer.receiveSamples(this.recv, avail);
        for (let i = 0; i < got && this.ringLen < this.ring.length; i++) {
          this.ring[this.ringLen++] = this.recv[2 * i]; // left channel -> mono
        }
      }
    }
    if (this.ringLen >= n) {
      output.set(this.ring.subarray(0, n));
      this.ring.copyWithin(0, n, this.ringLen);
      this.ringLen -= n;
    } else {
      output.set(this.ring.subarray(0, this.ringLen));
      output.fill(0, this.ringLen);
      this.ringLen = 0;
    }
    return true;
  }
}
registerProcessor("soundtouch-shifter", SoundTouchShifter);
